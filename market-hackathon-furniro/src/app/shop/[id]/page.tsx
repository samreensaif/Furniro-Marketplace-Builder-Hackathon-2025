...
async function Singleproduct({ searchParams }: { searchParams: Promise<{ 
  id: number,
  productName: string,
  productPrice: number,
  productImage: string,
  ...
  <div className="w-full flex items-center flex-wrap text-sm exsm:text-base sm:text-lg md:text-[20px] leading-[22px] exsm:leading-[24px] sm:leading-[28px] md:leading-[30px]">
   ...
   <p className="text-[#7c7474] ml-2 exsm:ml-3 sm:ml-4 md:ml-6 flex items-center">
    ...
    <div className="ml-2 exsm:ml-3 sm:ml-4 md:ml-6 pl-2 exsm:pl-3 sm:pl-4 md:pl-6 border-l-2 border-black">
      <p className="font-medium text-sm exsm:text-base sm:text-lg md:text-xl">{productName}</p>
    </div>
  </div>
</div>
      ...
      <Spmain id={id} productName={productName} productPrice={productPrice} productImage={productImage} productDescription={productDescription} dicountPercentage={productdicountPercentage} tags={tags} isNew={isNew}/>

      {/* Product Details Section */}
      <Productdetails id={id}tags={tags} isNew={isNew} productName={productName} productPrice={productPrice}productDescription={productDescription}  productImage={productImage} dicountPercentage={productdicountPercentage}  />
